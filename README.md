...
The mannequin is intentionally schematic: the Human style has simple fingers and facial details, but neither style simulates soft tissue, balance, grip forces or joint limits. Offline torso/head clearance correction is applied to the secondary HYP flows; the 108 book studies preserve authored joint planes and receive only a uniform floor translation; it is not a full collision or safety simulation. Some foot/hand contacts and historical configurations remain approximate. In particular, Simhasana's facial/finger detail is documented but not animated. The UI exposes these limits rather than claiming anatomical or historical precision.
